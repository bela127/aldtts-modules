...
class Pearson(DependencyTest):
    ...
    def pearsonr(self, x, y):
        ...
        dtype = (1.0 + x[0] + y[0]).dtype

        if n == 2:
            return dtype(np.sign(x[1] - x[0])*np.sign(y[1] - y[0])), 1.0

        xmean = x.mean(dtype=dtype, axis=0)[None,:]
        ymean = y.mean(dtype=dtype, axis=0)[None,:]
        ...
        normxm = scipy.linalg.norm(xm, axis = (0,))[None,:]
        normym = scipy.linalg.norm(ym, axis = (0,))[None,:]

        threshold = 1e-13
        if np.any(normxm < threshold*abs(xmean)) or np.any(normym < threshold*abs(ymean)):
            # If all the values in x (likewise y) are very close to the mean,
            # the loss of precision that occurs in the subtraction xm = x - xmean
            # might result in large errors in r.
            scipy.warnings.warn(scipy.stats.PearsonRNearConstantInputWarning())


        r = (xm/normxm).T @ (ym/normym)
        ...
        prob = np.reshape(prob, r.shape) #scipy.special.btdtr changes the shape in 1 dim cases, so chane it back
...
class Spearmanr(DependencyTest):
    ...
    def test(self):
        ...
        x = quries.T
        y = results.T
        t, p = np.vectorize(spearmanr, signature='(n),(n)->(),()')(x, y)
        #t, p = spearmanr(x, y)
...
class Kendalltau(DependencyTest):
    ...
    def test(self):
        ...
        x = quries.T
        y = results.T
        t, p = np.vectorize(kendalltau, signature='(n),(n)->(),()')(x, y)
        #t, p = kendalltau(x, y)
        tm = np.max(t, axis=0)
        pm = np.min(p, axis=0)
...
class Hoeffdings(DependencyTest):
    ...
    def test(self):
        ...
        x = queries.T
        y = results.T
        p = np.vectorize(hoeffding, signature='(n),(n)->()')(x, y)
        #p = hoeffding(x, y)
        tm = 0
        pm = np.min(p, axis=0)
